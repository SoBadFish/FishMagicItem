...
public class CustomTagData extends BaseDataWriterGetter<CustomTagItem> {
        ...
        if(item.hasCompoundTag()){
            if(item.getNamedTag().contains(TAG)){
                ...
                for(StringTag n: s.getAll()){
                    if(hasItem(n.parseValue())){
                        customTagItems.add(dataList.get(dataList.indexOf(CustomTagItem.asNameItem(n.parseValue()))));
                    }
                }
            }
        }
    ...
    public Item getItemByName(String name, TagController tagController){
        ...
        if(dataList.contains(tagItem)){
            ...
            //没有被tag存储的物品不是好物品
            //TODO
            resetItem(tagItem,item1);
            return chunkItem(tagItem,item1,tagController);
        }
        return null;
    }

    public Item chunkItem(CustomTagItem customTagItem,Item item,TagController tagController){
        if(!item.hasCompoundTag() || !item.getNamedTag().contains(TAG)){
...
//            tag.putList(TAG,new ListTag<StringTag>(new StringTag(customTagItem.name)));
    ...
    public Item resetItem(CustomTagItem customTagItem,Item item){
        ...
        //设置Lore
        String[] l2 = customTagItem.lore;
        for(int i = 0;i < l2.length;i++){
            l2[i] = TextFormat.colorize('&',l2[i]);
        }
        item.setLore(l2);
        ...
        for(String ench: customTagItem.enchant){
            String[] e = ench.split(":");
            Enchantment eh = Enchantment.getEnchantment(Integer.parseInt(e[0]));
            if(e.length > 1){
                eh.setLevel(Integer.parseInt(e[1]));
            }
    ...
    public void createTagItem(CustomTagItem customTagItem,Item item,TagController tagController){
        if(!item.hasCompoundTag() || !item.getNamedTag().contains(TAG)){
            chunkItem(customTagItem, item, tagController);
